...
struct context_VP8_s {
 ...
 //   VP8_FRAME_TYPE	picture_coding_mode;
    ...
    int8_t	BaselineFilter[MAX_MB_SEGMENTS]; 
...
inline psb_surface_stride_t get_opp_mode_stride(context_VP8_p ctx, const unsigned int uiStride)
{
...
static int get_inloop_opmod(context_VP8_p ctx)
{
	...
	psb_surface_p target_surface = ctx->obj_context->current_render_target->psb_surface;
	//printf("target_surface->stride=%d\n",target_surface->stride);
...
static void pnw_VP8_QueryConfigAttributes(
    VAProfile profile,
...
static VAStatus pnw_VP8_ValidateConfig(
...
static void pnw_VP8_DestroyContext(object_context_p obj_context);

static VAStatus pnw_VP8_CreateContext(
       ...
       object_config_p obj_config)
{
    ...
    uint32_t TotalMBs = (((obj_context->picture_height + 15) >> 4) + 4) * ((obj_context->picture_width + 15) >> 4);
    ctx->buffer_size = TotalMBs * 64;    // 64 bytes per MB 
    ctx->segid_size = TotalMBs / 4;      // 2 bits per MB
    ...
    if (vaStatus != VA_STATUS_SUCCESS) {
        pnw_VP8_DestroyContext(obj_context);
    }

    return vaStatus;
}

static void pnw_VP8_DestroyContext(
    object_context_p obj_context)
{
    INIT_CONTEXT_VP8
    int i;
...
static VAStatus pnw__VP8_allocate_colocated_buffer(context_VP8_p ctx, object_surface_p obj_surface, uint32_t size)
{
    ...
    if (!GET_SURFACE_INFO_colocated_index(surface)) {
        ...
        drv_debug_msg(VIDEO_DEBUG_GENERAL, "pnw_VP8: Allocating colocated buffer for surface %08x size = %08x\n", surface, size);
...
static VAStatus pnw__VP8_process_picture_param(context_VP8_p ctx, object_buffer_p obj_buffer)
{
    psb_surface_p target_surface = ctx->obj_context->current_render_target->psb_surface;
    uint32_t reg_value;
    VAStatus vaStatus;
    ...
    /* TODO : port DDK D3DDDIFMT_PICTUREPARAMSDATA ->  */
/*
...
*/
    ...
    vaStatus = pnw__VP8_allocate_colocated_buffer(ctx, ctx->obj_context->current_render_target, colocated_size);
    if (VA_STATUS_SUCCESS != vaStatus) {
        DEBUG_FAILURE;
        return vaStatus;
    }
...
static VAStatus pnw__VP8_process_probility_param(context_VP8_p ctx, object_buffer_p obj_buffer)
{
...
*/
static VAStatus pnw__VP8_add_slice_param(context_VP8_p ctx, object_buffer_p obj_buffer)
{
...
static void pnw__VP8_write_kick(context_VP8_p ctx, VASliceParameterBufferBase *slice_param)
{
...
static void pnw__VP8_FE_state(context_VP8_p ctx)
{
    psb_cmdbuf_p cmdbuf = ctx->obj_context->cmdbuf;
...
static void pnw__VP8_setup_alternative_frame(context_VP8_p ctx)
{
    uint32_t cmd;
    uint32_t alt_output_flag = 0;
    psb_cmdbuf_p cmdbuf = ctx->obj_context->cmdbuf;
...
/*
static void pnw__VP8_set_operation_target(context_VP8_p ctx)
{
    psb_cmdbuf_p cmdbuf = ctx->obj_context->cmdbuf;
    psb_surface_p target_surface = ctx->obj_context->current_render_target->psb_surface;
    ...
    pnw__VP8_setup_alternative_frame(ctx);
}
*/

static void pnw__VP8_set_target_picture(context_VP8_p ctx)
{
    psb_cmdbuf_p cmdbuf = ctx->obj_context->cmdbuf;
    psb_surface_p target_surface = ctx->obj_context->current_render_target->psb_surface;
...
static void pnw__VP8_set_reference_picture(context_VP8_p ctx)
{
...
/*
static void pnw__VP8_set_picture_header_data(context_VP8_p ctx)
{
    psb_cmdbuf_p cmdbuf = ctx->obj_context->cmdbuf;
    uint32_t reg_value;
...
static void pnw__VP8_compile_baseline_filter_data(context_VP8_p ctx)
{
    uint32_t i;

    for (i = 0; i < MAX_MB_SEGMENTS; i++)
    {
        ctx->BaselineFilter[i] = ctx->pic_params->loop_filter_level[i];
    }
}

static void pnw__VP8_compile_qindex_data(context_VP8_p ctx)
{
    uint32_t i;
    for (i = 0; i < MAX_MB_SEGMENTS; i++)
    {
...
static void pnw__CMDS_registers_write(context_VP8_p ctx)
{
    psb_cmdbuf_p cmdbuf = ctx->obj_context->cmdbuf;
    uint32_t reg_value;
   ...
   // psb_cmdbuf_reg_end_block(cmdbuf);
    ...
    reg_value = 0;
    REGIO_WRITE_FIELD_LITE( reg_value , MSVDX_CMDS, DISPLAY_PICTURE_SIZE, DISPLAY_PICTURE_WIDTH, ((ctx->pic_params->frame_width) - 1)  );
    REGIO_WRITE_FIELD_LITE( reg_value , MSVDX_CMDS, DISPLAY_PICTURE_SIZE, DISPLAY_PICTURE_HEIGHT, ((ctx->pic_params->frame_height) - 1) );
    psb_cmdbuf_rendec_write(cmdbuf, reg_value);
   // psb_cmdbuf_rendec_end(cmdbuf);
    ...
    reg_value = 0;
    REGIO_WRITE_FIELD_LITE( reg_value , MSVDX_CMDS, CODED_PICTURE_SIZE, CODED_PICTURE_WIDTH, ((ctx->pic_params->frame_width) - 1)  );
    REGIO_WRITE_FIELD_LITE( reg_value , MSVDX_CMDS, CODED_PICTURE_SIZE, CODED_PICTURE_HEIGHT, ((ctx->pic_params->frame_height) - 1) );
    psb_cmdbuf_rendec_write(cmdbuf, reg_value);
    ...
    REGIO_WRITE_FIELD_LITE( reg_value, MSVDX_CMDS,  VP8_LOOP_FILTER_CONTROL,  VP8_MODE_REF_LF_DELTA_ENABLED, ctx->pic_params->pic_fields.bits.sharpness_level);
    psb_cmdbuf_rendec_write(cmdbuf, reg_value);
    psb_cmdbuf_rendec_end(cmdbuf); 
    ...
    reg_value = 0;
    pnw__VP8_compile_baseline_filter_data(ctx);
    REGIO_WRITE_FIELD( reg_value, MSVDX_CMDS, VP8_LOOP_FILTER_BASELINE_LEVEL, VP8_LF_BASLINE3, ctx->BaselineFilter[3] );
    REGIO_WRITE_FIELD( reg_value, MSVDX_CMDS, VP8_LOOP_FILTER_BASELINE_LEVEL, VP8_LF_BASLINE2, ctx->BaselineFilter[2]);
    REGIO_WRITE_FIELD( reg_value, MSVDX_CMDS, VP8_LOOP_FILTER_BASELINE_LEVEL, VP8_LF_BASLINE1, ctx->BaselineFilter[1]);
    REGIO_WRITE_FIELD( reg_value, MSVDX_CMDS, VP8_LOOP_FILTER_BASELINE_LEVEL, VP8_LF_BASLINE0, ctx->BaselineFilter[0]);
    psb_cmdbuf_rendec_write(cmdbuf, reg_value);
    psb_cmdbuf_rendec_end(cmdbuf);
...
//    psb_cmdbuf_rendec_write_address(cmdbuf, &golden_ref_surface->buf, golden_ref_surface->buf.buffer_ofs + golden_ref_surface->chroma_offset);   
    ...
    pnw__VP8_setup_alternative_frame(ctx);  /* port from CVldDecoder::ProgramOutputModeRegisters */
}

static void pnw__VP8_set_slice_param(context_VP8_p ctx)
{
    psb_cmdbuf_p cmdbuf = ctx->obj_context->cmdbuf;
   // uint32_t cmd_slice_params;
    ...
    *ctx->p_slice_params = ctx->cmd_slice_params;
    ...
    /*if VP8 bitstream is multi-partitioned then convey all the info including buffer offest information for 2nd partiti     on to the firmware  */
...
static void pnw__VP8_FE_Registers_Write(context_VP8_p ctx)
{
    psb_cmdbuf_p cmdbuf = ctx->obj_context->cmdbuf;
    uint32_t reg_value;
    //uint32_t DCT_Base_Address = 0;
...
static void pnw__VP8_BE_Registers_Write(context_VP8_p ctx)
{
    psb_cmdbuf_p cmdbuf = ctx->obj_context->cmdbuf;
    uint32_t reg_value;
...
//        REGIO_WRITE_FIELD_LITE(reg_value, MSVDX_VEC_VP8, CR_VEC_VP8_BE_PIC2, VP8_BE_USE_STORED_SEGMENT_MAP, 1 );
...

    {
        ...
        pnw__VP8_compile_qindex_data(ctx);
            // QINDEX MAP
        reg_value = 0;
...
************************************************************************************/
static void pnw__VP8_set_bool_coder_context(context_VP8_p ctx)
{
	psb_cmdbuf_p cmdbuf = ctx->obj_context->cmdbuf;
...
************************************************************************************/
static void pnw_KeyFrame_BModeProbsDataCompile(Probability* ui8_probs_to_write, uint32_t* ui32_probs_buffer)
{
    uint32_t dim0, dim1;
...
* Description        : Write probability data in buffer according to MSVDX setting.
************************************************************************************/
static void pnw_InterFrame_YModeProbsDataCompile(Probability* ui8_probs_to_write, uint32_t* ui32_probs_buffer)
{
...
* Description        : Write probability data in buffer according to MSVDX setting.
************************************************************************************/
static void pnw_InterFrame_UVModeProbsDataCompile(Probability* ui8_probs_to_write, uint32_t* ui32_probs_buffer)
{
...
* Description        : Write probability data in buffer according to MSVDX setting.
************************************************************************************/
static void pnw_InterFrame_MVContextProbsDataCompile(uint8_t* ui8_probs_to_write, uint32_t* ui32_probs_buffer)
{
    uint32_t dim0;
    uint32_t i, j, address;
    uint32_t src_tab_offset = 0;
...
* Description        : Write probability data in buffer according to MSVDX setting.
************************************************************************************/
static void pnw_DCT_Coefficient_ProbsDataCompile(Probability* ui8_probs_to_write, uint32_t* ui32_probs_buffer)
{
...
************************************************************************************/
static void pnw__VP8_set_probility_reg(context_VP8_p ctx)
{
    psb_cmdbuf_p cmdbuf = ctx->obj_context->cmdbuf;
...
//    uint32_t* probs_table_1stbuffer = NULL;
    ...
    if(NULL == probs_buffer_1stPart) {
        drv_debug_msg(VIDEO_DEBUG_GENERAL, "pnw__VP8_set_probility_reg: map buffer fail\n");
        return;
     }
    {
        ...
        {
            pnw_KeyFrame_BModeProbsDataCompile(b_mode_prob, probs_buffer_1stPart);
        }
        else
        {
            pnw_InterFrame_YModeProbsDataCompile(ctx->pic_params->y_mode_probs, probs_buffer_1stPart);

            probs_buffer_1stPart += ( CABAC_LSR_InterFrame_UVModeProb_Address >> 2);
            pnw_InterFrame_UVModeProbsDataCompile(ctx->pic_params->uv_mode_probs, probs_buffer_1stPart);

            probs_buffer_1stPart += (CABAC_LSR_InterFrame_MVContextProb_Address >>2) - ( CABAC_LSR_InterFrame_UVModeProb_Address >> 2);
            pnw_InterFrame_MVContextProbsDataCompile(ctx->pic_params->mv_probs, probs_buffer_1stPart);
        }
...

    {
        ...
        pnw_DCT_Coefficient_ProbsDataCompile(ctx->probs_params->dct_coeff_probs, probs_buffer_2ndPart);

        psb_buffer_unmap(&ctx->probability_data_2nd_part);
	/* Set ui32ExternStateBuffAddr to addr of probability_data_2nd_part in pnw__VP8_FE_state() */

   }
}

//TODO port CVldDecoder::ProcessVLDSlice
static VAStatus pnw__VP8_process_slice(context_VP8_p ctx,
                                      VASliceParameterBufferBase *slice_param,
                                      object_buffer_p obj_buffer)
{
    ...
    //TODO port DDK CVP8VldDecoder::ProcessSlice here
        pnw__CMDS_registers_write(ctx);

        // Fill VP8 specific MSVDX FE registers (note: the behavior changes between partitions)
        pnw__VP8_FE_Registers_Write(ctx);

        // Fill VP8 BE registers
        pnw__VP8_BE_Registers_Write(ctx);

        pnw__VP8_set_slice_param(ctx);

        // Write  Probability registers
        pnw__VP8_set_probility_reg(ctx);//note

        pnw__VP8_set_target_picture(ctx);

        pnw__VP8_set_reference_picture(ctx);

        pnw__VP8_set_bool_coder_context(ctx); //note

        //pnw__VP8_set_decode_slice_reg(ctx);

        pnw__VP8_write_kick(ctx, slice_param);
...
static VAStatus pnw__VP8_process_slice_data(context_VP8_p ctx, object_buffer_p obj_buffer)
{
    VAStatus vaStatus = VA_STATUS_SUCCESS;
    VASliceParameterBufferBase *slice_param;
    int buffer_idx = 0;
    ...
    while (buffer_idx < ctx->slice_param_list_idx) {
        ...
        element_idx++;
        vaStatus = pnw__VP8_process_slice(ctx, slice_param, obj_buffer);
        if (vaStatus != VA_STATUS_SUCCESS) {
            DEBUG_FAILURE;
            break;
        }
...
static VAStatus pnw_VP8_BeginPicture(
    object_context_p obj_context)
{
    INIT_CONTEXT_VP8
...
static VAStatus pnw_VP8_RenderPicture(
    object_context_p obj_context,
    ...
    int num_buffers)
{
    ...
    for (i = 0; i < num_buffers; i++) {
        ...
        case VAPictureParameterBufferType:
            /* drv_debug_msg(VIDEO_DEBUG_GENERAL, "pnw_VP8_RenderPicture got VAPictureParameterBuffer\n"); */
            vaStatus = pnw__VP8_process_picture_param(ctx, obj_buffer);
            DEBUG_FAILURE;
            break;

        case VAProbabilityBufferType:
            /* drv_debug_msg(VIDEO_DEBUG_GENERAL, "pnw_VP8_RenderPicture got VAPictureParameterBuffer\n"); */
            vaStatus = pnw__VP8_process_probility_param(ctx, obj_buffer);
            DEBUG_FAILURE;
            break;

	case VAIQMatrixBufferType:
	    /* drv_debug_msg(VIDEO_DEBUG_GENERAL, "pnw_VP8_RenderPicture got VAIQMatrixBufferType\n"); */
	    vaStatus = psb__VP8_process_iq_matrix(ctx, obj_buffer);
	    DEBUG_FAILURE;
	    break;

        case VASliceParameterBufferType:
            /* drv_debug_msg(VIDEO_DEBUG_GENERAL, "pnw_VP8_RenderPicture got VASliceParameterBufferType\n"); */
            vaStatus = pnw__VP8_add_slice_param(ctx, obj_buffer);
            DEBUG_FAILURE;
            break;

        case VASliceDataBufferType:
        case VAProtectedSliceDataBufferType:
            /* drv_debug_msg(VIDEO_DEBUG_GENERAL, "pnw_VP8_RenderPicture got %s\n", SLICEDATA_BUFFER_TYPE(obj_buffer->type)); */
            vaStatus = pnw__VP8_process_slice_data(ctx, obj_buffer);
            DEBUG_FAILURE;
            break;
...
static VAStatus pnw_VP8_EndPicture(
    object_context_p obj_context)
{
    INIT_CONTEXT_VP8
...
struct format_vtable_s pnw_VP8_vtable = {
queryConfigAttributes:
    pnw_VP8_QueryConfigAttributes,
validateConfig:
    pnw_VP8_ValidateConfig,
createContext:
    pnw_VP8_CreateContext,
destroyContext:
    pnw_VP8_DestroyContext,
beginPicture:
    pnw_VP8_BeginPicture,
renderPicture:
    pnw_VP8_RenderPicture,
endPicture:
    pnw_VP8_EndPicture
};
